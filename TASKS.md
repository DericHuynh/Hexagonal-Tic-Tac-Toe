...
- [x] Create `turbo.json` with pipeline config (build, dev, test, typecheck)
...
- [x] Implement `countOpenLines(board: Board, coord: AxialCoord, player: Player, winLength: number, radius: number): number`
...
- [ ] `game/$id.tsx` — game page with HexCanvas, GameHUD, WebSocket integration (needs implementation)
...
- [ ] `api/auth/*` — BetterAuth routes (needs implementation)
...
### 6.1 Hex-to-Pixel Math (`src/lib/hex-to-pixel.ts`) ❌ **MISSING**
...
- [x] `pnpm test` passes with **0 failures** (401 tests as of latest run)
...
- **Phase 2**: All game-core logic complete and fully tested (401 passing tests)
